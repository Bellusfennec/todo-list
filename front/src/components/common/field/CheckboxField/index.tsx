...
const CheckboxField = (props: CheckboxFieldProps) => {
  ...
  return (
    <label className={"flex items-center" + (label ? " gap-[8px]" : "") + (className ? ` ${className}` : "")}>
      <input type="checkbox" name={name} checked={value} onChange={handlerChange} className="hidden" />
      <div className="cursor-pointer">
        {!value && <FaRegCircle className="w-6 h-6 text-gray-600 hover:text-cyan-800 transition duration-200" />}
